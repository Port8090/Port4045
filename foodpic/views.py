...
def create(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            image_instance.save()
            # YOLO 모델을 사용하여 객체 예측
            r = model.predict(source=image_instance.path.path)[0]  # 이미지 파일의 경로에 접근
            if len(r.boxes.cls) > 0:
                cls = r.names[int(r.boxes.cls[0])]  # 객체 클래스 추출
                result_image = Image.fromarray(r.plot()).convert('RGB')
                # 이미지 파일 이름 생성
                image_name = slugify(image_instance.path.name.split('/')[-1].split('.')[0])  # 파일 경로에서 파일 이름 추출 후 slugify
